...
class BlockchainAccount(models.Model):
    ...
    """
    _name = 'blockchain.account'
    _description = "Blockchain Account"
    ...
    connector_id = fields.Many2one('blockchain.connector', required=True)
    ...
    def action_generate(self):
        ...
        """
        action = self.env.ref('blockchain.blockchain_account_wizard_form_action')
        result = action.sudo().read()[0]
        return result

    def action_send(self):
        """
        Send blockchain wizard
        """
        action = self.env.ref('blockchain.blockchain_account_send_wizard_form_action')
        result = action.sudo().read()[0]
        return result

    def _action_send(self, password, to, amount):
        """
        Function to send blockchain to another account
        """
        self.ensure_one()
        blockchain_provider = Web3.HTTPProvider(self.connector_id.service_url)
        w3 = Web3(blockchain_provider)
        ...
        account_to = Web3.to_checksum_address(to)
        amount_in_wei = w3.to_wei(Decimal(str(amount)), 'ether')
        tx = {
            'nonce': nonce,
            'chainId': int(self.connector_id.chain), 
        ...
        try:
            key_data = ast.literal_eval(self.encrypted_key)
            privatekey = w3.eth.account.decrypt(key_data, password)
        except Exception as e:
            raise UserError(_("Không thể giải mã ví: Mật khẩu sai hoặc định dạng khóa lỗi. Chi tiết: %s") % str(e))
